...
dic_decode = {}
dic_decode['Aroma'] = 'review_aroma'
dic_decode['Aparência'] = 'review_visual'
dic_decode['Sabor'] = 'review_flavor'
dic_decode['Sensação'] = 'review_sensation'
dic_decode['Conjunto'] = 'review_general_set'
dic_decode['Avaliação Geral'] = 'review_general_rate'


# Function to get beer details
def get_beer_details(beer_url):
...
def get_beer_reviews(beer_url): 
    
    max_page_reviews = 999999
    review_page = 1
    beer_reviews = []
    
    full_url = ''
    while review_page < max_page_reviews:
...
def main():
    ...
    with open(f'output_{now_str}.csv', 'a') as f:
        while initial_beer_page <= max_beer_page:
            ...
            for item in beer_items:
                ...
                beer_details = [get_beer_details(beer_url)]
                if len(beer_details) == 0:
                    print(f'No details found for {beer_url}')
                    continue

                beer_reviews = get_beer_reviews(beer_url)
                ...
                for review in beer_reviews:
                    ...
                    if df_all is None:
                        ...
                        df_all = pd.concat([df_all, df_beer_rev], ignore_index=True)

               
                if item == beer_items[-1]:
                    initial_beer_page +=1
                
            df_all.to_csv(f, header=f.tell()==0, index=False)
            df_all = None
