...
class Step_2(Step):
    ...
    def select_rows_with_comment(self):
        ...
        self.inp_out_df = self.inp_out_df[self.inp_out_df["review_comment"].notna()]
    ...
    def sanitize_column(self, column_name, min_val, max_val):
        ...
        Returns:
            pandas.DataFrame: output dataframe
        """
        invalid_data = self.inp_out_df[
            (self.inp_out_df[column_name] > max_val) | (self.inp_out_df[column_name] < min_val)
        ]
        print(f"Removing {len(invalid_data)} lines of invalid {column_name}")
        self.inp_out_df = self.inp_out_df.drop(invalid_data.index)
    ...
    def run(self):
        ...
        self.inp_out_df.drop_duplicates(inplace=True)
        print(f"{len(self.inp_out_df)} lines Total")
        ...
        self.inp_out_df["review_datetime"] = pd.to_datetime(self.inp_out_df["review_datetime"])
        self.inp_out_df["beer_alcohol"] = (
            self.inp_out_df["beer_alcohol"].str.replace("% ABV", "").astype(float)
        )
        self.inp_out_df["beer_srm"] = (
            self.inp_out_df["beer_srm"].str.replace(".", "").str.replace(",", ".").astype(float)
        )
        for field in [
            "review_aroma",
            "review_visual",
            ...
            self.inp_out_df[field] = self.inp_out_df[field].apply(eval).astype(float)
            self.inp_out_df[field] = self.inp_out_df[field] * 5
        ...
        self.inp_out_df["review_comment_size"] = self.inp_out_df["review_comment"].str.len()
        
        # Looking for duplicates
        #
        dupl = self.inp_out_df
        ...
        col_keep = [col for col in self.inp_out_df.columns if col not in ['review_datetime']]
        self.inp_out_df.drop(dupl.index, inplace=True)
        print(f"Still {len(self.inp_out_df)} lines")
        ...
        garbage = self.inp_out_df[self.inp_out_df['review_comment_size'] < 4]
        ...
        self.inp_out_df.drop(garbage.index, inplace=True)
        print(f"Still {len(self.inp_out_df)} lines")
        ...
        #
        print('\nStatistics')
        print(self.inp_out_df.describe(percentiles))
        print(f'\nNumber of users: {len(self.inp_out_df["review_user"].unique())}')
        print(f'Number of reviews: {len(self.inp_out_df)}')
        print(f'Number of beers: {len(self.inp_out_df["beer_name"].unique())}')
        print(f'Number of breweries: {len(self.inp_out_df["beer_brewery_name"].unique())}')
        print(f'Number of styles: {len(self.inp_out_df["beer_style"].unique())}')

        unique_beer_count = self.inp_out_df.groupby('beer_brewery_name')['beer_name'].nunique().sort_values(ascending=False)
        print(f'\nTop Brewery beers produced:\n{unique_beer_count.head(20).to_string()}')

        unique_beer_count = self.inp_out_df.groupby('beer_brewery_name')['beer_style'].nunique().sort_values(ascending=False)
        print(f'\nTop Brewery styles produced:\n{unique_beer_count.to_string()}')

        beer_count = self.inp_out_df.groupby('beer_name')['beer_name'].count().sort_values(ascending=False)
        print(f'\nTop reviwed beers:\n{beer_count.head(100).to_string()}')
        print(beer_count.describe(percentiles))

        style_count = self.inp_out_df.groupby('beer_style')['beer_style'].count().sort_values(ascending=False)
        print(f'\nTop reviwed styles:\n{style_count.head(200).to_string()}')
        print(style_count.describe(percentiles))

        unique_active = self.inp_out_df.groupby('beer_is_active')['beer_is_active'].count().sort_values(ascending=False)
        print(f'\nActive beers:\n{unique_active.to_string()}')
        unique_sazonal = self.inp_out_df.groupby('beer_is_sazonal')['beer_is_sazonal'].count().sort_values(ascending=False)
        ...
        user_str = self.inp_out_df[self.inp_out_df['review_user'].isin(users_list)]
        ...
        self.inp_out_df['review_num_reviews'] = self.inp_out_df.groupby('review_user')['review_user'].transform('count')
