...
class Step_1(Step):
    ...
    def get_beer_details(self, beer_url):
        ...
        if field_val is None:
            return
        dic_beer_details["beer_name"] = field_val.find(
            "span", {"itemprop": "name"}
        ).text.strip()
    ...
    def get_beer_reviews(self, beer_url):
        ...
        while review_page < self.max_page_reviews:
            ...
            for item in review_items:
                ...
                # review_num_reviews
                field_reviews = field_row_user.find(
                    "span", {"class": "jrReviewerReviews"}
                )
                ...
                # review content div
                field_row_review_content = item.find(
                    "div", {"class": "jrReviewLayoutRight jrReviewContent"}
                )

                # review_datetime
                time_tag = field_row_review_content.find(
                    "time", {"class": "jrReviewCreated"}
                )
                ...
                field_row_review_rating = item.find(
                    "div", {"class": "jrRatingTable fwd-table"}
                )
                if field_row_review_rating is None:
                    continue

                rating_rows = field_row_review_rating.find_all(
                    "div", class_="fwd-table-row"
                )
                ...
                # review_comment
                field_row_review_comment = field_row_review_content.find(
                ...
                for item in beer_items:
                    ...
                    for review in beer_reviews:
                        ...
                        for key, value in review.items():
                            df_item[key] = [value]
                        df_beer_rev = pd.concat(
                            [
                                df_beer_details.reset_index(drop=True),
