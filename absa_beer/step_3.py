...
class Step_3(Step):
    ...
    def run(self):
        ...
        # First, select 16 reviews that matches the criteria to create the "Prompt Base Principal"
        ...
        reviews_for_prompt.to_csv(f'{self.work_dir}/step_3__reviews_for_prompt.csv', index=False)
...
pelo menos uma 'característica' e/ou uma ‘categoria’, que está relacionada as 'características' da cerveja. As categorias devem estar dentre os \
valores: ‘visual’, ‘aroma’, ‘sabor’ e ‘sensação na boca’. Selecione avaliações que contenham polaridade de ‘sentimento’ para \
as 'características' ou 'categorias'. Exemplo entre aspas: “Cerveja de coloração amarelo claro, líquido semi translúcido e espuma branca de boa \
formação. Aroma frutado, com as sensações de caju muito fortes e melão um pouco mais leve no segundo plano. Sabor azedo, salgado, com as \
predominâncias das frutas anunciadas no rótulo, caju e melão. Refrescante, álcool e amargor leves, bem carbonatada, corpo leve e retrogosto frutado.\
Gostei da combinação entre o salgado e adstringente do caju com o refrescante e levemente doce do melão. Mais uma cerveja de melão para conta!"\
Nesta avaliação, encontra-se as características (CC) e categorias entre [] abaixo:
[{"CC": "coloração amarelo claro", "categoria": "visual", "polaridade": "neutro"},
{"CC": "líquido semi translúcido", "categoria": "visual", "polaridade": "neutro"},
{"CC": "espuma branca de boa formação", "categoria": "visual", "polaridade": "positivo"},
{"CC": "aroma frutado", "categoria": "aroma", "polaridade": "positivo"},
{"CC": "caju muito fortes", "categoria": "aroma", "polaridade": "positivo"},
{"CC": "melão um pouco mais leve", "categoria": "aroma", "polaridade": "positivo"},
{"CC": "sabor azedo", "categoria": "sabor", "polaridade": "neutro"},
{"CC": "sabor salgado", "categoria": "sabor", "polaridade": "neutro"},
{"CC": "refrescante", "categoria": "sensação na boca", "polaridade": "positivo"},
{"CC": "álcool leve", "categoria": "sabor", "polaridade": "positivo"},
{"CC": "amargor leve", "categoria": "sabor", "polaridade": "positivo"},
{"CC": "bem carbonatada", "categoria": "sensação na boca", "polaridade": "positivo"},
{"CC": "corpo leve", "categoria": "sensação na boca", "polaridade": "positivo"},
{"CC": "retrogosto frutado", "categoria": "sabor", "polaridade": "positivo"},
{"CC": "gostei da combinação", "categoria": "sensação na boca", "polaridade": "muito positivo"}]
Exemplos de avaliações que poderão ser escolhidas, em uma lista compreendida entre ```, onde cada avaliação está entre ''' :"""
        ...
        reviews_eval = []
        reviews_comments = "```"
        prompt_user = """Selecione as avaliações abaixo, que estão em uma lista compreendida entre ```, onde cada avaliação está entre '''\
abaixo com base na informação enviada com instruções. Retorne no formato de uma lista [ índice, texto da avaliação, motivo], onde o \
motivo explica a escolha da avaliação. Retorne uma segunda lista, no formato [ índice, texto da avaliação, motivo], onde o motivo \
explica a não escolha da avaliação. """
        for i_general in range(0, 6):
            line = self.df.iloc[i_general]
            
            if review_eval_count == 5:
                prompt_user += reviews_comments + "```"
                response, finish_reason = get_completion(prompt_user, prompt_sys)
                ...
                reviews_comments += f"'''{line[['review_comment']].values[0]}'''"
                review_eval_count += 1
