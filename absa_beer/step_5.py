...
class Step_5(Step):
 ...
 # Function to clean and extract words/entities
    def extract_entities(self, text, stop_words, split_words=False):
        text = text.lower()
    ...
    def remove_obvious_words(self, aspect: str):
    ...
    def sort_two_words(self, aspect: str):
    ...
    def get_stop_words_sab_aro_sens_vis(self):
        ...
        for word in stop_words:
            new_word = self.sort_two_words(word)
    ...
    def generate_word_cloud(self, df: pd.DataFrame, base_name: str, stop_words: list, categories: list, max_words=50, split_words=False):
        ...
        word_list = df['aspect'].apply(lambda x: self.extract_entities(x, stop_words, split_words=split_words)).sum()
        ...
        for index, row in df.iterrows():
            words = self.extract_entities(row['aspect'], stop_words)
            for word in words:
    ...
    def get_most_common_word_by_year(self, df: pd.DataFrame, stop_words: list, sentiment: str):
        ...
        for y in range(1, len(years), 2):
            ...
            for category in categories:
                ...
                word_list = df_category['aspect'].apply(lambda x: self.extract_entities(x, stop_words)).sum()
                word_count = Counter(word_list)
                if word_count:
    ...
    def run(self):
        ...
        df_absa_join['aspect'] = df_absa_join['aspect'].apply(self.remove_obvious_words)
        ...
        df_absa_join.to_csv(f'{self.work_dir}/step_5_ABSA-FINAL.csv', index=False)
